...
- GitHub `main`へ反映し、Actions run `34669538247`で分割左右・片手左右・設定リセットの5ビルド、7件の`native_sim`試験、キーマップ図生成がすべて成功した。CI生成UF2は`keymap-gn-x-standalone-1to9-20260912/`へSHA-256・ビルド情報・案内とともに保存。
- 実機書き込みと実機入力は未確認。GitHub Release `v1.0.0`の版番号・公開済みファイルは更新していない。
